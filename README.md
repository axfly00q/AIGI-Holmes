...
- 🧠 **深度学习检测**：基于 ResNet-50 微调模型输出 AI 生成风险，并转换为“较可能真实 / 较可能 AI / 证据不足”三态结果；保留 Grad-CAM 热力图
- 🔬 **可选深度分析**：通过独立按钮运行印章、频域、边缘、人脸、Logo 与 EXIF 信号；这些信号仅供取证参考，不改变模型结论
...
| 功能模块 | 描述 |
|---|---|
| **单图检测** | 拖拽或上传图片，输出三态结论 + AI 生成风险 + Grad-CAM 热力图 |
...
| 指标 | 含义 |
|---|---|
| 判定结果 | `较可能由 AI 生成` / `较可能为真实照片` / `证据不足，暂无法判断` |
| AI 生成风险 | ResNet50 对 FAKE 类的校准后概率（0～100%） |
| 检测信号 | ResNet50 是结论依据；深度分析信号均标记为辅助参考 |
| 热力图 | Grad-CAM 可视化，高亮显示影响判定的图像区域 |
| 内容标签 | CLIP 零样本分类结果（如：人物、新闻现场） |
| 图文一致性 | 图片与配文的语义匹配度评分 |
| AI 分析 | 豆包 AI 对检测结果的自然语言解读 |

旧版本检测记录不做结果回填，统一标记为“旧模型”；需要新三态结果时请重新检测。

### 图像模型复现

图像模型使用 CIFAKE 官方的 100,000 张训练图，并从训练集固定划分 10,000 张验证图；官方 20,000 张测试集只用于最终评估。训练脚本会进行温度校准、三态阈值校准，激活新模型前自动备份旧权重。
...
当前模型的指标和评估范围以 `resources/image_detector/evaluation_current.yaml` 和模型卡为准。本轮按要求采用 1,000 张均衡抽样测试，不将其表述为完整 20,000 张成绩。CIFAKE 是 32×32 的课程数据集，其测试表现不能直接代表真实新闻图片的泛化能力。
