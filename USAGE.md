...
- **输出**：
  - 🔴 **较可能由 AI 生成** / 🟢 **较可能为真实照片** / 🟠 **证据不足**
  - AI 生成风险百分比
  - Grad-CAM 热力图（标记 AI 生成迹象）
  - 点击“深度分析”可额外查看取证信号；辅助信号不参与最终结论
  - 图片内容分类（人物/动物/建筑等）
...
- 不再使用未经实验记录支持的“约 95%”宣传值。
- 当前激活模型在 CIFAKE 官方测试划分中的 1,000 张均衡样本（FAKE/REAL 各 500）上的 Accuracy、Macro-F1、分类别 Precision/Recall、ROC-AUC、ECE、混淆矩阵和推理耗时，见 `resources/image_detector/evaluation_current.yaml`；不要将其写成完整测试集成绩。
- 旧模型历史记录会标记为“旧模型”，不与当前模型指标混用。

**影响因素**：
- CIFAKE 原图只有 32×32，课程测试表现不等同于真实新闻场景表现。
- 图片来源、压缩、裁剪、生成工具版本和跨平台处理都会造成分布偏移。
- “较可能真实”不是来源真实性证明，高风险结果也应经过人工复核。
